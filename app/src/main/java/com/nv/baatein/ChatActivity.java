...
public class ChatActivity extends AppCompatActivity {
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == 438 && resultCode == RESULT_OK && data.getData() != null) {
            ...
            loadingBar.setMessage("Photo Attachment");
            ...
            if (!checker.equals("image")) {
                ...
                filePath.putFile(filesUri).addOnCompleteListener(new OnCompleteListener<UploadTask.TaskSnapshot>() {
                    @Override
                    public void onComplete(@NonNull Task<UploadTask.TaskSnapshot> task) {
                        if (task.isSuccessful()) {
                            Map messageTextBody = new HashMap();
                            messageTextBody.put("message", task.getResult().getStorage().getDownloadUrl().toString());
                            messageTextBody.put("name", filesUri.getLastPathSegment());
                ...
                uploadTask = filePath.putFile(filesUri);
